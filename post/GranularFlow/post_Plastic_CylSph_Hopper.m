...
7.01, 88.49];
 ...
 folder=['../'  '../output/Test_Plastic_Cylinder_Sphere/Hopper/'];
% folder=['../'  '../output/Test_Plastic_Sphere_Cylinder/Hopper/'];
...
for i=1:1:699
    ...
    z=data.Z+data.r;
    family=data.family;
    totalMassCyl=numel(find(family==familyCyl))/cylinder*massCyl;
    totalMassSph=numel(find(family==familySph))*massSph;
    totalMass=totalMassCyl+totalMassSph;
    index=find(z<0.00 );
    vec=z(index);
    x=x(index);
    y=y(index);
    ...
    if numel(index)>1
        ...
        % disp(value)
            totalMassCyl=numel(find(family==familyCyl))/cylinder*massCyl;
    totalMassSph=numel(find(family==familySph))*massSph;
    totalMassTemp=totalMassCyl+totalMassSph;
