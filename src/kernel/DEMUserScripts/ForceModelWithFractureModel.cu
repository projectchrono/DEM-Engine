...
if (unbroken > DEME_TINY_FLOAT) {
    ...
    float3 bond_force = -B2A;
    float force_mag = 1000;

    force += bond_force * force_mag;
...
// Whether or not the bonding force among particle components exists, contact forces are always active if two particles
// are in contact: This is a part of our model.
if (overlapDepth > 0) {
    // Material properties
