...
// You can notice that unlike ForceModelWithElectrostatic.cu, we did not enclose everything in a
// if (overlapDepth > 0) bracket. This is because we did not SetFamilyExtraMargin in the use case
// if this force model, so the solver will not try to process contacts that are not in physical contact.
// Since our cohesion model works when there is physical contact, too, there is no need to do extra checking.
{
