...
if (overlapDepth > 0) {
    ...
    printf(
        "Area is %f, cnt_rad is %f, penetration is %f, Sn is %f, k_n is %f, gamma_n is %f, force is %f %f %f, B2A is "
        "%f %f %f\n",
        overlapArea, cnt_rad, overlapDepth, Sn, k_n, gamma_n, force.x, force.y, force.z, B2A.x, B2A.y, B2A.z);
}
