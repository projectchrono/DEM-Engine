...
__global__ void rearrangeContactWildcards(sgps::contactPairs_t* contactMapping,
                                          float* oldWildcards[],
                                          float* newWildcards[],
    ...
    if (myID < nContactPairs) {
        ...
        if (map_from == sgps::DEM_NULL_MAPPING_PARTNER) {
            ...
            for (unsigned int i = 0; i < nWildcards; i++) {
                newWildcards[i][myID] = 0;
            }
        } else {
            // Not a new contact, need to map it from somewhere in the old history array
            for (unsigned int i = 0; i < nWildcards; i++) {
                newWildcards[i][myID] = oldWildcards[i][map_from];
            }
