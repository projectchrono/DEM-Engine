...
std::filesystem::path JitHelper::resolveCacheDir() {
    // Could be handy to save this quite big compile time for e.g. simple C++ compilations etc.
    if (const char* env = std::getenv("DEME_JIT_CACHE_DIR")) {
        return std::filesystem::path(env);
    }
    // Prefer to keep cache alongside other runtime data in the build tree
    std::filesystem::path default_path = RuntimeDataHelper::data_path / "jit_cache";
    if (std::filesystem::exists(RuntimeDataHelper::data_path)) {
        return default_path;
    }
