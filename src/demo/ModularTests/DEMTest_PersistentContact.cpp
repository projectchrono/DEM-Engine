...
int main() {
    ...
    // Load materials
    auto mat_type_box = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", 0.6}, {"mu", 0.4}});
    auto mat_type_plane = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", 0.6}, {"mu", 0.4}});
    ...
    std::cout << "\n--- Creating Plane 1 (left) ---" << std::endl;
    // Create first plane (left side)
    auto plane1 = DEMSim.AddMeshFromTemplate(plane_template, make_float3(-5, 0, 0));
    ...
    std::cout << "\n--- Creating Plane 2 (right) ---" << std::endl;
    // Create second plane (right side)
    auto plane2 = DEMSim.AddMeshFromTemplate(plane_template, make_float3(5, 0, 0));
    plane2->SetFamily(11);  // Plane family 2
    plane2->SetMass(10000.);
    plane2->SetMOI(make_float3(10000., 10000., 10000.));
    DEMSim.SetFamilyFixed(11);  // Planes are fixed in place
    ...
    for (int i = 0; i < 3; i++) {
        float x = -5.0f + i * 1.2f;
        float y = -2.0f + i * 2.0f;
        float z = 1.0f;  // Start above plane
        ...
        plane1_box_trackers.push_back(tracker);
