...
int main(int argc, char* argv[]) {
    ...
    std::string out_dir = "/DemoOutput_Force2D/";
...
void runDEME(std::string dir_output, float frictionMaterial, float massMultiplier) {
    ...
    // E, nu, CoR, mu, Crr...
    auto mat_type_terrain = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.33}, {"CoR", 0.5}, {"mu", frictionMaterial}, {"Crr", 0.0}});
    ...
    templates_terrain.push_back(
        DEMSim.LoadSphereType(terrain_rad * terrain_rad * 1.0e3 * PI, terrain_rad, mat_type_terrain));

    templates_terrain.push_back(
        DEMSim.LoadSphereType(terrain_rad * terrain_rad * massMultiplier * 1.0e3 * PI, terrain_rad, mat_type_terrain));
