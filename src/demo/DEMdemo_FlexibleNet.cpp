...
int main() {
    ...
    float sphere_rad = 0.025;

    float step_size = 2e-6;
    float fact_radius = 1.0;

    DEMSim.InstructBoxDomainDimension({-30 , 30},
                                      {-10, 10}, {-12, 10});
    ...
    auto data_xyz = DEMSim.ReadClumpXyzFromCsv("../data/clumps/net_flex.csv");
    ...
    top_plane->SetInitPos(make_float3(0, 0, -0.50));
    top_plane->SetMass(1.);
    top_plane->Scale(make_float3(1, 5, 0.01));
    ...
    bottom_plane->SetInitPos(make_float3(0, 0, -12.0));
    ...
    out_dir += "/DemoOutput_Flexible_Net";
    remove_all(out_dir);
