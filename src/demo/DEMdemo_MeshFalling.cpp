...
int main() {
    ...
    // Avoid per-contact force/point recording (can be very expensive when many mesh contacts exist).
    DEMSim.SetNoForceRecord();
    const bool print_contact_stats = false;
    DEMSim.InstructBoxDomainDimension({-15.f, 15.f}, {-15.f, 15.f}, {-10.f, 25.f});
    ...
    auto mat_box = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", 0.2}, {"mu", 0.4}, {"Crr", 0.1}});
    auto mat_plane = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", 0.2}, {"mu", 0.3}, {"Crr", 0.1}});
    ...
    const float particle_spacing = 2.5f;
    const float initial_height = 4.0f;
    const float layer_height_offset = 3.0f;
    ...
    std::uniform_real_distribution<> pos_dist(-0.05, 0.05);
    ...
    for (int layer = 0; layer < num_layers; layer++) {
        for (int i = 0; i < num_particles_x; i++) {
            for (int j = 0; j < num_particles_y; j++) {
                ...
                float z = initial_height + layer * layer_height_offset + (i + j) * 0.5 + pos_dist(gen) * 2;
    ...
    DEMSim.SetErrorOutAvgContacts(1500.0f);
    DEMSim.SetCDMaxUpdateFreq(20);
    ...
    std::cout << "Starting simulation with " << num_layers * num_particles_x * num_particles_y
              << " particles (diverse meshes)" << std::endl;
    std::cout << "Frame time: " << frame_time << " s, Time step: " << step_time << " s" << std::endl;
    std::cout << "Total frames: " << (int)(final_time / frame_time) << std::endl;

    const int total_frames = (int)(final_time / frame_time);
    for (frame = 0; frame <= total_frames; frame++) {
        const double t = frame * frame_time;
        std::cout << "Frame: " << frame << " (t = " << t << " s)" << std::endl;
        ...
        if (print_contact_stats) {
            std::cout << "  Num contacts: " << DEMSim.GetNumContacts() << std::endl;
            std::cout << "  Avg primitive contacts: " << DEMSim.GetAvgPrimitiveContacts() << std::endl;
            std::cout << "  Bin size: " << DEMSim.GetBinSize() << std::endl;
            std::cout << "  Bin num: " << DEMSim.GetBinNum() << std::endl;
        }

        DEMSim.ShowMemStats();
        std::cout << "----------------------------------------" << std::endl;

        // Advance to the next output frame (internally still integrates with `step_time`).
