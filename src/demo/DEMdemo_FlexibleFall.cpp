...
int main() {
    ...
    float3 CylAxis = make_float3(0, 0, 0.9);
    float CylRad = 0.50;
    float CylHeight = 4;
    ...
    DEMSim.AddFamilyPrescribedAcc(4, to_string_with_precision(0.), to_string_with_precision(0.),
                                  to_string_with_precision(-4000.));

    auto cube = DEMSim.AddWavefrontMeshObject("../data/granularFlow/funnel_left.obj", mat_type_granular_2);
    float3 move = make_float3(1.5, 0.00, 5);  // z
    float4 rot = make_float4(1, 0, 0, 0);
    cube->Scale(make_float3(20, 60, 8.0));
    cube->Move(move, rot);
    cube->SetFamily(20);
    cube->SetMass(5000.);
    DEMSim.AddFamilyPrescribedAcc(20, to_string_with_precision(0.), to_string_with_precision(0.), "none");
    ...
    //  Settle
    for (double t = 0; t < 0.00; t += frame_time) {
