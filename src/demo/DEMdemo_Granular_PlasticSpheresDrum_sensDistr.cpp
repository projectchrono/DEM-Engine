...
int main(){
    ...
    std::vector<float> friction = {0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70};
    std::vector<float> distribution = {0.00, 0.01, 0.02, 0.04, 0.08};
        
    int counter=0;

    for (int i = 0; i < int(friction.size()); i++) {
        for (int j = 0; j < int(distribution.size()); j++) {
            runDEME(counter, friction[i], distribution[j]);
            counter++;
        }
    }
...
void runDEME(int caseDef, float frictionMaterial,float sigma) {
    DEMSolver DEMSim;
    ...
    out_dir += "/DemoOutput_Granular_PlasticSpheresDrum_sensDistr_";
   ...
   auto fixed = DEMSim.AddWavefrontMeshObject("../data/granularFlow/drum.obj", mat_type_walls);     
    ...
    std::normal_distribution<double> distribution(radius,radius*sigma);
    double maxRadius= 0;
