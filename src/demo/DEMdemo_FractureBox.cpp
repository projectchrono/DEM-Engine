...
int main() {
    ...
    auto mat_type_particle = DEMSim.LoadMaterial({{"E", 60e9}, {"nu", 0.5}, {"CoR", 0.5}, {"mu", 1.0}, {"Crr", 0.05}});
    ...
    float sphere_rad = 0.0011;
    float step_size = 25e-8;
    ...
    DEMSim.SetFamilyPrescribedLinVel(11, "0", "0", to_string_with_precision(-0.0050));

    auto cylinder = DEMSim.AddExternalObject();
    cylinder->AddCylinder(make_float3(0), make_float3(0, 0, 1), 2 * container_diameter / 2., mat_type_container, 0);
    ...
    move = make_float3(0.05, 0.00, 0.10+1.5*sphere_rad);  // z
    ...
    DEMSim.SetFamilyExtraMargin(1, 1.2* sphere_rad);
    ...
    for (float t = 0; t < sim_end; t += frame_time) {
        ...
        if (frame_count % 1 == 0) {
            ...
            std::cout << "Stress [Pa]: " << forces.z / (deme::PI * container_diameter * container_diameter / 4)
                      << std::endl;
