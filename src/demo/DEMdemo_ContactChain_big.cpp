...
int main() {
    ...
    float massMultiplier = 200.0;  // Magnitude of the external force

    std::cout << "Inner friction: " << innerFriction << "; Mass multiplier: " << massMultiplier << "." << std::endl;
    path out_dir = "";
    out_dir += "./ContactChainBig_out_";
    remove_all(out_dir);
    ...
    float step_size = 0.01 * DTc;
    double world_sizeX = 1004 * 2 * terrain_rad;  // 122.0 works fine for frictionless
    double world_sizeZ = 886 * terrain_rad;

    DEMSim.InstructBoxDomainDimension({-world_sizeX / 2., world_sizeX / 2.}, {-5 * terrain_rad, 5 * terrain_rad},
                                      {-1 * world_sizeZ, 100 * terrain_rad});
    ...
    auto data_xyz = DEMSim.ReadClumpXyzFromCsv("../data/clumps/ContactChainBig_initial.csv");
    ...
    float sim_time = 2.0 + timeApplication;
