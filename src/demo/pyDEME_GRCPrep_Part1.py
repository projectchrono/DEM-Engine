...
if __name__ == "__main__":
    ...
    my_template2 = DEMSim.LoadClumpType(mass2, MOI2, GetDEMEDataFile("clumps/triangular_flat_6comp.csv"), mat_type_terrain)
    my_template1 = DEMSim.LoadClumpType(mass1, MOI1, GetDEMEDataFile("clumps/triangular_flat.csv"), mat_type_terrain)
    ground_particle_templates = [my_template2,
    ...
    # Sampler to use
    sampler = HCPSampler(scale[0] * 2.2)
    ...
    while (DEMSim.GetNumClumps() < 0.25e6):
        ...
        heap_particles_xyz = sampler.SampleBox(sample_center, [sample_halfwidth_x, sample_halfwidth_y, sample_halfheight])

        # Pick from possible grain templates with a pre-defined possibility
        heap_family = np.random.choice(range(len(grain_perc)), len(heap_particles_xyz), p=grain_perc)
        heap_template_in_use = [ground_particle_templates[i] for i in heap_family]
        
        heap_particles = DEMSim.AddClumps(heap_template_in_use, heap_particles_xyz)
    ...
    printf("DEMdemo_GRCPrep_Part1 exiting...")
