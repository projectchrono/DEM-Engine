...
int main() {
    ...
    if (loaded) {
        ...
        float thresholds[] = {10.0f, 30.0f, 45.0f, 90.0f};
        
        for (float threshold : thresholds) {
            size_t num_patches = cube_mesh->SplitIntoConvexPatches(threshold);
            std::cout << "\nAngle threshold: " << std::fixed << std::setprecision(1) << threshold << " degrees" << std::endl;
    ...
    std::cout << "Patches computed: " << (empty_mesh->ArePatchesComputed() ? "yes" : "no") << " (expected: no)" << std::endl;
