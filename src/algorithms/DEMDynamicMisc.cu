...
__global__ void getContactForcesConcerningOwners_impl(float3* d_points,
    ...
    /*  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
       if (i<numCnt) {
           ...
           size_t writeIndex = atomicAdd(d_numUsefulCnt, 1);
                   float3 cntPnt;
           double3 CoM;
           float4 oriQ;
           if (AorB) {
           ...
           if (need_torque) {
               ...
               if (!torque_in_local) {  // back to global if needed
                   applyOriQToVector3<float, deme::oriQ_t>(torque.x, torque.y, torque.z, oriQ.w, oriQ.x, oriQ.y,
       oriQ.z);
               }
           }
       ...
       simParams->voxelSize, simParams->l); CoM.x += simParams->LBFX; CoM.y += simParams->LBFY; CoM.z +=
       simParams->LBFZ; applyFrameTransformLocalToGlobal<float3, double3, float4>(cntPnt, CoM, oriQ);
           d_points[writeIndex] = cntPnt;
           d_forces[writeIndex] = force;
           if (need_torque) d_torques[writeIndex] = torque;
       }
   */
}
...
void getContactForcesConcerningOwners(float3* d_points,
    ...
    getContactForcesConcerningOwners_impl<<<blocks_needed, DEME_MAX_THREADS_PER_BLOCK, 0, this_stream>>>(
        d_points, d_forces, d_torques, d_numUsefulCnt, d_ownerIDs, IDListSize, simParams, granData, numCnt, need_torque,
        torque_in_local);
