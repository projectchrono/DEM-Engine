...
PYBIND11_MODULE(_deme, obj) {
    ...
    if (py::hasattr(obj, "__file__")) {
        path = std::filesystem::path(py::str(obj.attr("__file__"))).parent_path().parent_path();
    } else {
        py::module_ site = py::module_::import("site");
        path = site.attr("getsitepackages")().cast<py::list>()[0].cast<std::string>();
    }

    // Set runtime paths relative to the installed extension rather than the process working directory.
    DEMERuntimeDataHelper::SetPathPrefix(path);
    deme::SetDEMEDataPath((DEMERuntimeDataHelper::data_path / "data").string());
    ...
    py::class_<deme::DEMTracker, std::shared_ptr<deme::DEMTracker>>(obj, "Tracker")
        ...
        .def(
            ...
            [](deme::DEMTracker& tracker, size_t offset) {
                std::vector<float3> points, forces, torques;
                tracker.GetContactForcesAndLocalTorque(points, forces, torques, offset);
                return py::make_tuple(points, forces, torques);
            },
        ...
        .def(
            ...
            [](deme::DEMTracker& tracker) {
                std::vector<float3> points, forces, torques;
                tracker.GetContactForcesAndLocalTorqueForAll(points, forces, torques);
                return py::make_tuple(points, forces, torques);
            },
            "Get all contact forces and local torques that concern all objects tracked by this tracker, as a list.")

        .def(
            "GetContactForcesAndGlobalTorque",
            [](deme::DEMTracker& tracker, size_t offset) {
                std::vector<float3> points, forces, torques;
                tracker.GetContactForcesAndGlobalTorque(points, forces, torques, offset);
                return py::make_tuple(points, forces, torques);
            },
        ...
        .def(
            ...
            [](deme::DEMTracker& tracker) {
                std::vector<float3> points, forces, torques;
                tracker.GetContactForcesAndGlobalTorqueForAll(points, forces, torques);
                return py::make_tuple(points, forces, torques);
            },
        ...
        .def(
            ...
            [](deme::DEMTracker& tracker, size_t offset) {
                ...
                tracker.GetContactForces(points, forces, offset);
                return py::make_tuple(points, forces);
            },
            "Get all contact forces that concern this tracked object, as a list.", py::arg("offset") = 0)
        .def(
            "GetContactForcesForAll",
            [](deme::DEMTracker& tracker) {
                std::vector<float3> points, forces;
                tracker.GetContactForcesForAll(points, forces);
                return py::make_tuple(points, forces);
            },
    ...
    py::class_<deme::DEMClumpTemplate, std::shared_ptr<deme::DEMClumpTemplate>>(obj, "DEMClumpTemplate")
        .def(py::init<>())
        .def("Mass", [](const deme::DEMClumpTemplate& clump) { return clump.mass; })
        .def("MOI", [](const deme::DEMClumpTemplate& clump) { return clump.MOI; })
    ...
    py::class_<deme::DEMExternObj, deme::DEMInitializer, std::shared_ptr<deme::DEMExternObj>>(obj, "DEMExternObj")
        .def(py::init<>())
        .def("Mass", [](const deme::DEMExternObj& object) { return object.mass; })
        .def("MOI", [](const deme::DEMExternObj& object) { return object.MOI; })
    ...
    py::class_<deme::DEMMeshConnected, deme::DEMInitializer, std::shared_ptr<deme::DEMMeshConnected>>(
        ...
        .def("Mass", [](const deme::DEMMeshConnected& mesh) { return mesh.mass; })
        .def("MOI", [](const deme::DEMMeshConnected& mesh) { return mesh.MOI; })
